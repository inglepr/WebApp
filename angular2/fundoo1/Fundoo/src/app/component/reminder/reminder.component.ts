...
export class ReminderComponent implements OnInit {
 ...
 dateValue: DateValueComp = new DateValueComp();
    ...
    getnotes() {
      this.noteService.getRequest("getReminderNote").subscribe(
        (response: any) => {
          this.note = response;
    ...
    addLabelToNote(labels, items) {
      ...
      this.labelService.putRequest("addLabelToNote?labelId=" + labels.labelId + "&noteId=" + items.noteId, "").subscribe((response: any) => {
        if (response.statusCode === 200) {
    ...
    getAllLabels() {
  // this.noteService.getRequest("getlabel").subscribe(
